...
use crustache::Data;
use crustache::{compile_str, MapBuilder, VecBuilder};
...
#[test]
fn test_map_fn2_builder() {
    let tpl = compile_str("{{#x}}{{y}}{{/x}}").expect("compile");
    let data = MapBuilder::new()
        .insert_str("y", "z")
        .insert_fn2("x", |_section_src, render| render("{{y}}".to_string()))
        .build();
    assert_eq!(tpl.render_data_to_string(&data).expect("render"), "z");
}
