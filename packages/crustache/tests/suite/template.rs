...
use crustache::{self, Data, Error, to_data};
use crustache::{Context, Template};
...
fn compile_str(s: &str) -> Template {
    crustache::compile_str(s).expect(&format!("Failed to compile: {}", s))
}
...
fn assert_partials_data(template: Template) {
    ...
    // Partials supplied as `Data::Template` (nickel-org/rust-mustache#80).
    let mut ctx0 = HashMap::new();
    let mut ctx1 = HashMap::new();
    ctx1.insert("name".to_string(), Data::String("a".to_string()));
    ctx0.insert("names".to_string(), Data::Vec(vec![Data::Map(ctx1)]));
    let user_partial = crustache::compile_str("<span>{{>username}}</span>\n").unwrap();
...
fn test_render_partial_dot_filename() {
    let template = crustache::compile_path("tests/test-data/base.foo.mustache").expect("Failed to compile");
    assert_partials_data(template);
}

#[test]
fn test_render_partial() {
    let template = crustache::compile_path("tests/test-data/base").expect("Failed to compile");
    assert_partials_data(template);
}
...
#[test]
#[ignore = "Requires mustache/spec at spec/ (https://github.com/mustache/spec)"]
