...
use crate::{to_data, Data, Template};
...
impl MapBuilder {
    ...
    /// Inserts a compiled [`Template`] as data so `{{> key}}` can resolve it before filesystem partials.
    ///
    /// ```rust
    /// use crustache::{compile_str, MapBuilder};
    /// let user_tpl = compile_str("<strong>{{username}}</strong>").unwrap();
    /// let data = MapBuilder::new()
    ///     .insert_template("user", user_tpl)
    ///     .build();
    /// ```
    #[inline]
    pub fn insert_template<K: ToString>(self, key: K, value: Template) -> MapBuilder {
        let MapBuilder { mut data } = self;
        data.insert(key.to_string(), Data::Template(value));
        MapBuilder { data: data }
    }

    /// Like [`MapBuilder::insert_fn`], but the closure receives a render callback for the section body.
    #[inline]
...
impl VecBuilder {
    ...
    #[inline]
    pub fn push_fn2<F>(self, f: F) -> VecBuilder
    where
        F: FnMut(String, &mut dyn FnMut(String) -> String) -> String + Send + 'static,
    {
        let VecBuilder { mut data } = self;
        data.push(Data::Fun2(RefCell::new(Box::new(f))));
        VecBuilder { data: data }
    }
